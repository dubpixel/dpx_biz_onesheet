...
## PROJECT: dpx_biz_onesheet

**Purpose:** Interactive quick-reference web page for the dpx_biz capacitive touch PCB business card
**Version File:** `src/version.js` (per-page versions, centralized)

### Architecture (2-minute summary)

Standalone, dependency-free HTML/CSS/JS site. Each page hovers over the PCB image to show pad function (boot combos, HID mappings, MIDI notes, LED colors). No build step — `src/` is the source of truth, `deploy.sh` copies finished files into `web/`, which GitHub Pages serves.

| Component | Location | Purpose | Notes |
|-----------|----------|---------|-------|
| Main interactive page | `src/dpx-interactive.html` | Hover-map onesheet (Web Serial enabled) | Deploys to `web/index.html` |
| No-serial variant | `src/dpx-interactive-noSerial.html` | Same page without Web Serial API | For browsers without serial support |
...
| Firmware binary | `web/dpx_biz_frm_v0d5d7.uf2` | Card firmware for download | Served directly from `web/` |
| Jekyll config | `_config.yml` | GitHub Pages / kramdown settings | Root of repo |
| Reference docs | `src/reference_documents/` | Pad mapping, boot timing, style guide, planning docs | See `context.md` and `instructions.md` there for the full pad→GPIO→LED→HID/MIDI table — don't duplicate it here |
| Archive | `src/zzz_archive/` | Superseded page/doc versions kept for history | Not deployed; gitignored (`.gitignore` has both `zz_archive` and `zzz_archive` patterns) |

### Deployment

Two separate publish targets exist, and only one has any config in this repo:

| Target | Trigger | Config location |
|--------|---------|------------------|
| **Cloudflare Pages** (`dpx-biz-onesheet.pages.dev`, linked as "Live Demo" in README) | Auto-deploys on push, likely to `main` — connected via Cloudflare's own GitHub integration, **not** any file in this repo | Cloudflare dashboard only. No `wrangler.toml`, `_redirects`, `_headers`, or Pages-related GitHub Action exists here. Output directory is believed to be `web/` but this is unconfirmed from the repo side — check the Cloudflare Pages project settings if the live site stops matching `web/`. |
| **GitHub Pages / Jekyll** | Standard GitHub Pages build on push to the Pages-configured branch | `_config.yml` (root) + `Gemfile` — kramdown/remote-theme settings |

Run `./deploy.sh` locally first either way — it's what populates `web/`, which is what both targets ultimately serve.

### Working Loop (how changes actually ship here)

There is no CI/build pipeline in this repo — pushing `main` is what goes live, so the human-in-the-loop preview step happens locally, every time:

1. Make the change in `src/`
2. Run `./deploy.sh` to sync it into `web/`
3. Open the affected `web/*.html` file directly in a browser so the user can preview it (this is the "spin it up" step — there's no dev server, just opening the static file)
4. **Wait for explicit user approval** before committing/pushing — don't push speculatively
5. On approval: commit, push to `main`
6. Cloudflare Pages picks up the push automatically and deploys — no further action needed, but also no repo-visible confirmation that it succeeded (check the dashboard or the live URL if verification matters)

### Key Decisions

- **`src/` vs `web/` split**: `src/` is edited by hand; `web/` is a deploy target populated by `deploy.sh`. Never hand-edit files in `web/` — edit `src/` and redeploy.
- **No build step**: plain HTML/CSS/JS on purpose — the card firmware ships without internet access, so the reference page must work offline by opening the file directly.
- **Per-page versions in `src/version.js`**: pages have independent version numbers (not one repo-wide version) because they update on different cadences.
- **Default pad mappings target disguise d3 designer** (not DaVinci Resolve or any other app) — see `src/reference_documents/instructions.md` for the explicit correction; don't reintroduce DaVinci Resolve references.
...
**Add a new page:** Add to `src/`, wire it into `deploy.sh`'s copy list, add a version entry in `src/version.js`
...
- Start of session: `gh issue list --repo dubpixel/dpx_biz_onesheet`
- End of session: `gh issue create --repo dubpixel/dpx_biz_onesheet --title "..." --body "..."`
...
2. **Update version number** in `src/version.js` (per page)
...
- This project: per-page entries in `src/version.js`
...
Examples:
- `[Interactive] Fix tooltip clipping on narrow portrait`
- `[Docs] Consolidate architecture documentation`
- `[Deploy] Fix version.js copy step`
...
- Adjust comment syntax for the language (`#` for Python/bash, `//` for JS/C++, `<!-- -->` for HTML)
...
Project-specific architecture, decisions, and operational knowledge should live in this AGENTS.md file (see PROJECT section above). This keeps rules and context unified in one scannable document.
...
If you create CONTEXT.md for overflow, add a reference in the PROJECT section: "See CONTEXT.md for full pad mapping reference."
...
- README.md must be updated and maintained when appropriate as per these guidelines
- CONTEXT.md must be updated and maintained when appropriate as per these guidelines
- a comprehensive CHANGELOG.md must be kept updated and maintained when appropriate as per these guidelines
...
- changelog could be retroactively updated to reflect git commit names if that adds clarity
...
- HTML/CSS/JS: no framework, no build step — keep it dependency-free and readable in a plain browser
...
- Verify the code works (open the page in a browser)
...
- [ ] Test/verify the changes (open in browser)
